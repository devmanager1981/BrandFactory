...
2. **Pixel Difference Heatmap (NEW):** Crops the generated image to the product region and compares it pixel-by-pixel against the original Master Product image to generate a difference score and a visual heatmap overlay for the UI [New].
...
*For any* generated image with a pixel difference heatmap, the consistency score must be a value between 0.0 and 1.0 (or 0% to 100%).
